...
def tran_e(stress, T_n, fc):
    k2 = 2.35  # k2 is between 1.8 and 2.35
    if 20 <= T_n <= 700:
        therm_strain = -1.8 * 10 ** (-4) + 9 * 10 ** (-6) * T_n + 2.3 * 10 ** (-11) * T_n ** 3
    elif 700 <= T_n:
        therm_strain = 14 * 10 ** (-3)
    ...
    tran_strain = k2 * stress * therm_strain / fc
...
def cre_e(stress, T_n, fc, time):
    ...
    temp_term = math.e ** (0.00304 * (T_n - 20))
    creep_strain = 5.3 * stress * (time / 180) ** 0.5 * temp_term / fc / 10000
...
@jit
def solver(time, dt, p_a, h_0, s, e_0, height, width, cover, dx, D_sr, N_com_bar, N_ten_bar, fys, Es, fc, fct, Pa, ecc, length, BC):
    ...
    biaxial_loading = np.zeros(size_strain + 1)
            ...
            if n == interval * q:
                ...
                for k in range(0, size_strain + 1):
                    ...
                    if strain_limit[k] > 0:
                        ...
                        for i in range(1, Ny + 1):
                            ...
                            for j in range(0, i + 1):
                                ...
                                mom_ini_c_c[j] = width * dx ** 3 / 12 + (width * dx) * (height / 2 - (i - j) * dx) ** 2    # inividual second moment of each layer: I
                                ...
                                buckling_c_c[j][k] = (math.pi ** 2) * modulus_c_c[j][k] * mom_ini_c_c[j] / (length * BC) ** 2
                        ...
                        for i in range(0, Nx + 1):
                            for j in range(0, Ny + 1):
                                ...
                                c_euler += c_mod[i][j] * c_mom_ini[i][j]
                        ...
                        I_steel = math.pi * D_sr ** 4 / 64 + (math.pi * D_sr ** 2 / 4) * (height / 2 - cover) ** 2
                    ...
                    if strain_limit[k] < 0:
                        ...
                        for i in range(1, Ny + 1):
                            mid_deflec[i] = i * dx / strain_limit[k] * (1 - math.cos(length * strain_limit[k] / (2 * i * dx)))
                    ...
                    #biaxial loading result
                    biaxial_loading[k] = 1 / (1 / loading[k] * 2 - 1 / load_failure[Ny + 1])

                    #uniaxial loading result
...
solver(time, dt, p_a, h_0, s, e_0, height, width, cover, dx, D_sr, N_com_bar, N_ten_bar, fys, Es, fc, fct, Pa, ecc, length, BC)